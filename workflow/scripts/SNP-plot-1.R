...
get_allele <- function(genotype, alt, ps, haplotype) {
  if (genotype == "1|0" || genotype == "1/0" && haplotype == 1) {
    return(alt)
  } else if (genotype == "0|1" || genotype == "0/1" && haplotype == 2) {
    return(alt)
...
manual_color_mapping <- c("A" = "#8ff035", "T" = "#f70a0a", "C" = "#1060ec", "G" = "#ffc445", "NA" = "white")
...
p <- ggplot(snp_df, aes(x = SNP, y = Haplotype, fill = Allele)) +
...
#Run this if just making one plot
#filtered_df <- final_df %>% dplyr::filter(GENE_EXON %in% include_gene_exon)
